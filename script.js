...
// ==========================================
// BẢN VÁ TỐI ƯU: GỬI SOS BẰNG ĐỊNH VỊ GPS
// ==========================================
window.submitSupportRequest = () => { 
    ...
    const sendDataToFirebase = (lat, lng) => {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            ...
            (err) => sendDataToFirebase(null, null),
            { timeout: 5000 }
        );
    } else { sendDataToFirebase(null, null); }
};
...
window.checkSupportReply = () => {
    ...
    window.db.ref('replies').once('value').then(snap => {
        ...
        snap.forEach(c => {
            const d = c.val();
            if(d.name.toLowerCase() === n && d.secret === s) {
                window.showCustomAlert("📩 PHẢN HỒI CỦA ANH QUÂN 🥰", d.msg, "✅");
...
window.loadTracking = () => { 
    ...
    // ------------------------------------------
    // 1. TẢI BẢNG THEO DÕI ONLINE / OFFLINE (GIỮ NGUYÊN)
    // ------------------------------------------
    ...
    // ------------------------------------------
    // 2. TẢI NHẬT KÝ SỬA ĐIỂM (GIỮ NGUYÊN)
    // ------------------------------------------
    ...
    // ------------------------------------------
    // 3. TẢI HỘP THƯ HỖ TRỢ SOS (ĐÃ GẮN LINK GOOGLE MAPS)
    // ------------------------------------------
    ...
    window.db.ref('inbox').on('value', snap => {
        ...
        reqArray.forEach(req => { 
            ...
            let mapHtml = '';
            if (req.lat && req.lng) {
                const mapLink = `https://www.google.com/maps/search/?api=1&query=${req.lat},${req.lng}`;
                mapHtml = `<br><b style="color:#4CAF50;">📍 Vị trí:</b> <a href="${mapLink}" target="_blank" style="color:#4CAF50; font-weight:bold; text-decoration:underline; font-size:13px;">Mở Bản Đồ</a>`;
            } else {
                mapHtml = `<br><b style="color:#dc3545;">📍 Vị trí:</b> <span style="font-size:12px; color:#dc3545; font-style:italic;">Bị từ chối GPS</span>`;
            }
