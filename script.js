...
// --- HÀM HIỆN BẢNG CHÍNH SÁCH ---
window.showPolicyContent = () => {
    const noiDung = `
1. Giữ bí mật,mật khẩu cá nhân Các em nhé!.
2. Không dùng lời lẽ thiếu văn hóa trong chat Riêng và nhóm.
3. Các em nhớ để ý Thông báo ở trong web nhé.
4. Tôn trọng giáo viên và các bạn.
------------------------------------
Các em yên tâm nhé, Nếu các em mà không làm gì sai không phá web không chọc mạch cái gì thì anh sẽ không truy xét lưu ý nhé🥰
Anh chúc các em học tập vui vẻ nhé! 🥰
    `;
    // Gọi hàm thông báo có sẵn trong code của bạn
    window.showCustomAlert('NỘI QUY LỚP HỌC', noiDung, '📜');
};

// --- HÀM XỬ LÝ ĐĂNG NHẬP ---
window.handleLogin = () => {
    // Bước 1: Kiểm tra xem đã tích vào ô đồng ý chưa
    const isAgreed = document.getElementById('policy-agree').checked;
    if (!isAgreed) {
        return window.showCustomAlert('ANH NHẮC EM', 'Em ơi, nhớ tích vào ô "Đồng ý với chính sách" thì mới vào lớp được nhé em!', '⚠️');
    }

    // Bước 2: Lấy dữ liệu đăng nhập
    const i = document.getElementById('username').value.trim().toLowerCase(); 
    const p = document.getElementById('password').value.trim(); 
    const b = document.getElementById('login-btn');
    
    if (!i || !p) return window.showCustomAlert('LỖI', 'Vui lòng điền đủ ID và Mật khẩu!', '⚠️'); 
    
    b.innerText = "ĐANG KIỂM TRA..."; 
    b.disabled = true;
    ...
    firebase.auth().signInWithEmailAndPassword(emailAo, p).then(() => {
        window.db.ref('users/' + i).once('value').then(s => {
            if (i === 'admin') { 
                window.session = { id: i, role: 'admin', name: 'ANH QUÂN' }; 
                window.startIntro();
            } else if (s.exists()) {
                const d = s.val();
                if (d.isLocked) { 
                    window.showCustomAlert('KHÓA 🔒', 'Tài khoản đã bị khóa!', '🚫');
                    firebase.auth().signOut(); 
                    b.innerText = "VÀO HỆ THỐNG 🚀"; b.disabled = false;
                } else {
                    window.session = { id: i, role: d.role, name: d.name }; 
                    window.db.ref('tracking/' + i).update({ status: 'online' }); 
                    window.startIntro();
                }
            } else {
                window.showCustomAlert('LỖI', 'Tài khoản không tồn tại!', '❌');
                firebase.auth().signOut();
                b.innerText = "VÀO HỆ THỐNG 🚀"; b.disabled = false;
            }
        });
        ...
        b.innerText = "VÀO HỆ THỐNG 🚀"; b.disabled = false; 
        window.showCustomAlert('THẤT BẠI', 'Sai ID hoặc Mật khẩu rồi em ơi!', '❌');
    });
};
