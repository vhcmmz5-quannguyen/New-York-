...
window.handleLogin = () => {
    ...
    firebase.auth().signInWithEmailAndPassword(emailAo, p).then(() => {
        // Đăng nhập thành công (giữ nguyên logic cũ của bạn)
        ...
        // QUAN TRỌNG: Trả lại trạng thái nút ngay lập tức khi sai thông tin
        b.innerText = "VÀO HỆ THỐNG 🚀"; 
        b.disabled = false; 

        // Hiển thị thông báo hồng chuẩn hệ thống
...
// XUẤT TOÀN BỘ BẢNG ĐIỂM RA FILE CSV (MỞ ĐƯỢC BẰNG EXCEL)
window.exportExcel = () => {
    ...
    let csv = "\uFEFF"; // Dùng BOM để Excel hiểu định dạng tiếng Việt (UTF-8)
    for (let i = 0; i < table.rows.length; i++) {
        let row = [], cols = table.rows[i].cells;
        for (let j = 0; j < cols.length; j++) {
            // Loại bỏ các ký tự xuống dòng và bao quanh bằng dấu ngoặc kép để tránh lỗi định dạng
    ...
    link.setAttribute("download", "Bang_Diem_Tong_Ket_" + window.getDateStr() + ".csv");
...
// HÀM HIỂN THỊ TIN NHẮN (ĐA NĂNG) - ĐÃ VÁ LỖI BẢO MẬT XSS
window.renderMessage = (msg, isMe, msgKey, type, convoId) => {
    ...
    if(txtHtml === '[UNSENT]') {
        ...
        txtHtml = `<img src="${safeUrl}" style="max-height:180px; max-width:100%; border-radius:10px; cursor:pointer; display:block; margin-top:5px;" onclick="window.viewFullImage('${safeUrl}')">`;
    } else {
        txtHtml = window.escapeHTML(txtHtml);
    }

    const isAdmin = window.session && window.session.role === 'admin';
    const unsendBtn = ((isMe || isAdmin) && msg.text !== '[UNSENT]') ? `<span onclick="window.unsendMsg('${type}', '${convoId}', '${msgKey}')" style="font-size:12px; cursor:pointer; margin-left:10px; color:var(--text-light);">🗑️</span>` : '';
    
    const reactBtn = (msg.text !== '[UNSENT]') ? `<span onclick="window.openReactionPicker('${type}', '${convoId}', '${msgKey}')" style="font-size:12px; cursor:pointer; margin-left:10px; color:var(--text-light); opacity:0.8;">😀</span>` : '';
    ...
    if (msg.reactions) {
        ...
        reactionHtml = `<div style="position:absolute; bottom:-12px; ${isMe ? 'left:10px;' : 'right:10px;'} background:var(--card); border:1px solid var(--border); border-radius:12px; padding:2px 6px; font-size:11px; box-shadow:0 2px 5px rgba(0,0,0,0.1); display:flex; gap:3px; z-index:2; cursor:pointer;" onclick="window.openReactionPicker('${type}', '${convoId}', '${msgKey}')">`;
        for(let r in counts) reactionHtml += `<span>${r} ${counts[r]}</span>`;
        reactionHtml += `</div>`;
    }

    const dName = (window.myNicknames && window.myNicknames[msg.id]) ? window.myNicknames[msg.id] : msg.name;

    // BẢN VÁ: Gắn ID để cập nhật độc lập từng tin nhắn
    return `<div id="msg-${msgKey}" class="${bgClass}" style="max-width:85%; ${align} ${defaultBg} padding:8px 12px; border-radius:15px; position:relative; margin-bottom:15px;">
        <div style="font-size:10px; font-weight:bold; margin-bottom:3px; color:${nameColor};" onclick="window.openUserProfile('${msg.id}')">${dName}</div>
        <div style="font-size:14px; word-break:break-word;">${txtHtml}${reactBtn}${unsendBtn}</div>
        ${reactionHtml}
    </div>`;
};
...
window.openUserActionMenu = (id, name, pass, isLocked, targetRole) => {
    if (window.session.role === 'gv' && (targetRole === 'gv' || id === 'admin')) { 
        return window.showCustomAlert('🚫 BỊ CHẶN: Bạn không có quyền can thiệp đồng nghiệp!'); 
    }
    document.getElementById('action-u-name').innerText = name + " (" + id.toUpperCase() + ")";
    document.getElementById('btn-action-edit').onclick = () => { window.toggleModal('user-action-modal', false); window.openEditUser(id, name, pass); };
    
    const lockBtn = document.getElementById('btn-action-lock');
    if (isLocked) { lockBtn.innerHTML = "🔓 Mở Khóa"; lockBtn.style.background = "#4CAF50"; } 
    else { lockBtn.innerHTML = "🔒 Khóa"; lockBtn.style.background = "#FF9800"; }
    
    lockBtn.onclick = () => { window.toggleModal('user-action-modal', false); window.clickToggleLock(id, name, isLocked); };
    document.getElementById('btn-action-delete').onclick = () => { window.toggleModal('user-action-modal', false); window.clickDelete(id, name); };
...
window.confirmDeleteUser = () => { 
    ...
    window.db.ref('deleted_logs/' + id).set({ reason: reason, time: window.now() }); 
    
    let up = {}; 
    up['users/' + id] = null; 
    up['user_passwords/' + id] = null; 
    up['grades/' + id] = null; 
    up['tracking/' + id] = null; 
    up['friends/' + id] = null;
    
    window.db.ref('groups').once('value').then(snap => {
        snap.forEach(group => {
            const grpData = group.val();
            if (grpData.members && grpData.members[id]) {
                up[`groups/${group.key}/members/${id}`] = null;
            }
        });

        window.db.ref().update(up).then(() => { 
            if(typeof window.showCustomAlert === 'function') {
                window.showCustomAlert('THÀNH CÔNG', 'Đã xóa vĩnh viễn tài khoản và dọn dẹp dữ liệu!', '✅'); 
            } else {
                window.showCustomAlert("✅ Đã xóa vĩnh viễn tài khoản!"); 
            }
    ...
    window.db.ref('inbox').on('value', snap => {
        ...
        for (let k in data) { 
            ...
            html += `<div style="background:var(--bg); padding:15px; border-radius:10px; border-left:4px solid #FF9800; border:1px solid var(--border); margin-bottom:10px;">
                ...
                <div style="font-size:13px;"><b>Vấn đề:</b> <span style="color:#dc3545;">${req.req}</span><br><b>Mã:</b> ${req.secret}</div>
                <button class="btn-royal" style="background:#FF9800; margin-top:10px; padding:8px; font-size:12px;" onclick="window.replySupport('${k}', '${window.escapeHTML(req.name)}', '${req.secret}')">✍️ TRẢ LỜI</button>
            </div>`; 
        }
...
window.replySupport = (reqKey, reqName, secret) => {
    const msg = prompt("Nhập câu trả lời cho " + reqName + ":");
    if(!msg) return;
    window.db.ref('replies/' + Date.now()).set({ name: reqName, secret: secret, msg: msg }).then(() => {
        window.db.ref('inbox/' + reqKey).remove();
        window.showCustomAlert("✅ Đã phản hồi thành công!");
    });
};
...
// Hàm Mở bảng biểu cảm
window.openReactionPicker = (type, convoId, msgKey) => {
    window.currentReactionTarget = { type, convoId, msgKey };
    window.toggleModal('reaction-modal', true);
};

// Hàm Gửi biểu cảm lên Firebase
window.sendReaction = (emoji) => {
    ...
    window.toggleModal('reaction-modal', false);
};
