...
// 1. Hàm hiện bảng nội quy - Đã thêm e.stopPropagation() để cô lập cú click
window.showPolicyContent = (e) => {
    if (e) {
        e.preventDefault();
        e.stopPropagation(); // Chặn không cho lệnh click ăn lan làm thay đổi ô checkbox phía sau
    }
    
    const policyText = `
1. Giữ bí mật mật khẩu cá nhân.
2. Không dùng ngôn từ thiếu văn hóa trong lớp.
3. Anh chị có quyền khoá/xoá nếu các em vi phạm các hành vi trên các em nhé!.
4. Tôn trọng giáo viên và các bạn cùng lớp.
--------------------------------------------
Chúc các em xem điểm và hoạt động vui vẻ nha🥰
    `;
    window.showCustomAlert('CHÍNH SÁCH LỚP HỌC', policyText, '📜');
};
window.handleLogin = () => {
    ...
    const agreeCheck = document.getElementById('policy-agree');
    if (agreeCheck && !agreeCheck.checked) {
        return window.showCustomAlert('ANH NHẮC EM🫵📜', 'Em ơi, nhớ tích vào ô "Tôi đồng ý với chính sách của lớp" thì mới vào hệ thống được nhé!', '⚠️');
    }
        ...
        window.db.ref('users/' + i).once('value').then(s => {
            if (i === 'admin') { 
                ...
                if (d.isLocked) { 
                    ...
                    // BẢN VÁ: Gắn giá trị mặc định chống sập App nếu Firebase mất dữ liệu
                    window.session = { 
                        id: i, 
                        role: d.role || 'hs', // Nếu không có quyền, tự động ép về Học Sinh
                        name: d.name || i.toUpperCase(), // Nếu mất tên, lấy ID làm tên
...
window.enterApp = () => {
    try {
        console.log("🎬 Đang kích hoạt hàm enterApp...");
        if(typeof window.requestNoti === 'function') window.requestNoti();

        // 1. ÉP MÀN HÌNH CHÍNH PHẢI HIỂN THỊ - XÓA BỎ HOÀN TOÀN MÀN HÌNH TRẮNG
        const mainScreen = document.getElementById('main-screen');
        if (mainScreen) {
            mainScreen.classList.remove('hidden');
            mainScreen.style.setProperty('display', 'block', 'important'); // Ghi đè tối cao vào CSS
        } else {
            alert("🚨 Không tìm thấy thẻ có id='main-screen' trong index.html!");
        }

        // 2. Tạo dữ liệu an toàn tránh sập ứng dụng khi Firebase chưa trả về kịp
        const safeRole = window.session?.role || 'hs';
        const safeName = window.session?.name || window.session?.id?.toUpperCase() || 'THÀNH VIÊN';

        // 3. Kiểm tra và đổ dữ liệu an toàn (Không lo thiếu thẻ HTML)
        const elName = document.getElementById('display-name-real');
        if (elName) elName.innerText = safeName;

        const elRole = document.getElementById('display-role');
        if (elRole) elRole.innerText = safeRole.toUpperCase();

        const elAvatar = document.getElementById('user-avatar');
        if (elAvatar) elAvatar.src = window.session?.avatar || 'https://cdn-icons-png.flaticon.com/512/149/149071.png';

        const editorZone = document.getElementById('rules-editor-zone');
        if (editorZone) {
            if (safeRole === 'admin' || safeRole === 'gv') {
        ...
        // 4. Phân chia menu chức năng (Tabs)
        ...
        // Ẩn toàn bộ nút hướng điều hướng trước
        document.querySelectorAll('.nav-btn').forEach(b => {
            if (!b.onclick || !b.onclick.toString().includes('handleLogout')) {
                b.classList.add('hidden');
            }
        });

        // Bật lại các nút được cấp quyền hợp lệ
        let activeTabs = (safeRole === 'admin' || window.isDemoMode) ? adminTabs : (safeRole === 'gv' ? gvTabs : hsTabs);
        activeTabs.forEach(id => {
            const btn = document.getElementById(id);
            if (btn) btn.classList.remove('hidden');
        });

        // Xử lý riêng cho quyền Giáo viên
        if (safeRole === 'gv') {
        ...
        // 5. Điều hướng phân hệ mặc định khi vừa đăng nhập thành công
        if (safeRole === 'admin' || window.isDemoMode) {
        ...
        // Khởi tạo mã QR kết nối cá nhân công khai
        const qrEl = document.getElementById('connect-my-qr');
        if (qrEl) {
            qrEl.src = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=user=${window.session?.id || 'unknown'}`;
        }

        // Kích hoạt các luồng dữ liệu thời gian thực phụ trợ
        ...
        // 6. Lắng nghe thông báo tin nhắn chưa đọc từ Firebase
        if (window.session?.id) {
            window.db.ref('unread/' + window.session.id).on('value', s => {
                ...
                ['main-noti-dot', 'menu-noti-dot', 'private-noti-dot'].forEach(id => {
        ...
        // Kiểm tra điều hướng sâu qua liên kết URL (Deep link xem trang cá nhân)
        const urlParams = new URLSearchParams(window.location.search);
        const targetUser = urlParams.get('user');
        if (targetUser && window.session?.id && targetUser !== window.session.id && typeof window.openUserProfile === 'function') {
            window.openUserProfile(targetUser);
        }

    } catch (e) {
        alert("🚨 SẬP LÔGIC TẠI ENTERAPP:\n" + e.message + "\n\nChi tiết vị trí:\n" + e.stack);
    }
};
...
window.loadFriendRequests = () => {
    // 1. Kiểm tra an toàn: Nếu Firebase chưa khởi tạo xong thì dừng ngay
    if (!window.db || !window.session || !window.session.id) {
        console.warn("⚠️ Firebase hoặc Session chưa sẵn sàng, bỏ qua loadFriendRequests.");
        return;
    }

    // 2. Lắng nghe dữ liệu yêu cầu kết bạn theo thời gian thực
    window.db.ref('friend_requests/' + window.session.id).on('value', async (snap) => {
        const list = snap.val();
        const container = document.getElementById('friend-requests-list'); // Tên ID của cái bảng danh sách kết bạn trong HTML của mày
        
        if (!container) return; // Nếu không tìm thấy cái bảng danh sách trong HTML thì thôi
        
        container.innerHTML = ''; // Xóa trắng danh sách cũ để render mới

        if (!list) {
            container.innerHTML = '<p style="text-align:center; color:gray;">Chưa có lời mời kết bạn nào.</p>';
            return;
        }

        // 3. Render danh sách lời mời
        for (const reqKey in list) {
            const req = list[reqKey];
            const senderId = req.from; // ID người gửi
            
            // Lấy thông tin người gửi (sử dụng hàm getSafeUserInfo để tránh crash)
            const senderInfo = await window.getSafeUserInfo(senderId); 
            
            const div = document.createElement('div');
            div.className = 'friend-request-item';
            div.innerHTML = `
                <div class="user-info">
                    <img src="${senderInfo.avatar || 'https://cdn-icons-png.flaticon.com/512/149/149071.png'}" width="40" height="40" style="border-radius:50%">
                    <span>${senderInfo.name || 'Người dùng ẩn danh'}</span>
                </div>
                <div class="actions">
                    <button onclick="window.acceptFriend('${reqKey}', '${senderId}')" class="btn-accept">Đồng ý</button>
                    <button onclick="window.declineFriend('${reqKey}')" class="btn-decline">Từ chối</button>
                </div>
            `;
...
window.deleteAnnouncement = (id) => {
    ...
    if (confirm("🚨 Bạn có chắc chắn muốn gỡ bỏ hoàn toàn thông báo này không?")) {
