...
// ==========================================
// HỆ THỐNG ĐIỂM DANH TUẦN BẤT TẬN & NIÊM PHONG
// ==========================================
...
window.loadAttendanceSessions = () => {
    ...
    // Hiện nút thêm tuần cho Admin/GV
    ...
    window.db.ref('attendance_weeks').on('value', snap => {
        ...
        arr.sort((a, b) => a.time - b.time); // Tuần cũ trước, tuần mới sau

        arr.forEach(w => {
            const lockIcon = w.isLocked ? '<i class="fas fa-lock" style="font-size:10px; margin-left:5px;"></i>' : '';
            const activeClass = window.currentAttSession === w.id ? 'border: 2px solid var(--pink);' : '';
            html += `
                <div class="card shadow-lux flex-center" 
                     onclick="window.openAttendanceWeek('${w.id}', '${w.name}')"
                     style="padding:15px 5px; cursor:pointer; min-height:80px; text-align:center; transition:0.3s; ${activeClass}">
                    <b style="font-size:12px; color:var(--pink);">${w.name.toUpperCase()}</b>
                    <small style="font-size:10px; color:var(--text-light);">${w.date}</small>
                    <span style="font-size:10px;">${lockIcon}</span>
                </div>`;
        });
        
        const listEl = document.getElementById('attendance-week-list');
        if (listEl) listEl.innerHTML = html || '<p style="grid-column:1/-1; text-align:center; color:#888;">Chưa có dữ liệu. Bấm Thêm Tuần để bắt đầu!</p>';
    });
};

// 2. TẠO TUẦN MỚI (Tự động đề xuất số tuần tiếp theo)
window.createNewWeek = () => {
    window.db.ref('attendance_weeks').once('value').then(snap => {
        const nextNum = snap.numChildren() + 1;
        const name = prompt("Nhập tên tuần học:", "Tuần " + nextNum);
        if (!name) return;
        ...
        window.db.ref('attendance_weeks/' + weekId).set({
            ...
            window.showToast("✅ Đã tạo " + name);
            window.openAttendanceWeek(weekId, name);
        });
    });
};

// 3. XEM CHI TIẾT ĐIỂM DANH MỘT TUẦN
window.openAttendanceWeek = (id, name) => {
    ...
    document.getElementById('att-current-week-title').innerText = "ĐANG XEM: " + name.toUpperCase();
    ...
    window.db.ref('attendance_weeks/' + id).on('value', s => {
        ...
        if (btnLock) {
            ...
            if (window.isAttSessionLocked) {
                btnLock.innerHTML = window.session.role === 'admin' ? "🔓 MỞ KHÓA" : "🔒 ĐÃ CHỐT";
                btnLock.style.background = window.session.role === 'admin' ? "#4CAF50" : "#888";
            } else {
                btnLock.innerHTML = "🔒 CHỐT SỔ";
    ...
    window.db.ref('attendance_data/' + id).on('value', sData => {
        ...
        window.db.ref('users').once('value').then(sUsers => {
            ...
            for (let uid in users) {
                if (users[uid].role !== 'hs') continue;
                const status = data[uid];
                let stTxt = '<span style="color:#888; font-size:11px;">Chưa ĐD</span>';
                if (status === 'co_mat') stTxt = '<b style="color:#4CAF50;">🟢 CÓ MẶT</b>';
                if (status === 'nghi') stTxt = '<b style="color:#dc3545;">🔴 NGHỈ</b>';

                html += `<tr onclick="window.openMarkMenu('${uid}', '${window.escapeHTML(users[uid].name)}')" style="cursor:pointer;">
                    <td style="text-align:left; padding-left:15px;"><b>${window.escapeHTML(users[uid].name)}</b><br><small>ID: ${uid.toUpperCase()}</small></td>
                    <td style="text-align:center;">${stTxt}</td>
                </tr>`;
            }
...
// 4. MENU ĐIỂM DANH (Có chặn quyền nếu đã Chốt)
window.openMarkMenu = (uid, name) => {
    // CHẶN: Nếu bảng đã chốt và không phải Admin thì cấm sửa
    if (window.isAttSessionLocked && window.session.role !== 'admin') {
        if(typeof window.showCustomAlert === 'function') {
            return window.showCustomAlert("BẢNG ĐÃ KHÓA 🔒", "Tuần này đã được chốt sổ. Chỉ Boss mới có quyền sửa đổi!", "🚫");
        } else {
            return alert("Bảng đã khóa! Vui lòng báo Boss để sửa.");
        }
    }
...
window.saveAttendance = (status) => {
    ...
    if (status === 'clear') {
        window.db.ref(`attendance_data/${sid}/${uid}`).remove();
    } else {
        window.db.ref(`attendance_data/${sid}/${uid}`).set(status);
    }
    window.toggleModal('attendance-action-modal', false);
};

// 5. CHỐT / MỞ KHÓA (Chỉ Admin mới có "chìa khóa" mở lại)
window.toggleSessionLock = () => {
    if (window.isAttSessionLocked && window.session.role !== 'admin') {
        if(typeof window.showCustomAlert === 'function') {
            return window.showCustomAlert("KHÔNG CÓ QUYỀN", "Bạn không có chìa khóa để mở lại bảng này!", "🚨");
        } else {
            return alert("Chỉ Boss mới mở khóa được!");
        }
    }
    ...
    const msg = newStatus ? "Bạn có chắc chắn muốn CHỐT SỔ tuần này? Giáo viên sẽ không thể sửa đổi nữa." : "Xác nhận MỞ KHÓA để giáo viên sửa đổi?";

    if (confirm(msg)) {
        window.db.ref('attendance_weeks/' + sid).update({ isLocked: newStatus }).then(() => {
            if(typeof window.showToast === 'function') {
                window.showToast(newStatus ? "🔒 Đã chốt sổ!" : "🔓 Đã mở khóa!");
            }
        });
    }
};
