...
// XUẤT TOÀN BỘ BẢNG ĐIỂM RA FILE EXCEL CHUẨN (.XLSX)
window.exportExcel = () => {
    ...
    try {
        // Dùng thư viện XLSX để chuyển bảng HTML thành dữ liệu Excel chuẩn
        const workbook = XLSX.utils.table_to_book(table, { sheet: "Bảng Điểm" });
        
        // Tạo tên file kèm ngày tháng
        ...
        // Báo thành công
        if(typeof window.showCustomAlert === 'function') {
            window.showCustomAlert('THÀNH CÔNG', 'Đã tải xuống file Excel chuẩn định dạng!', '✅');
        }
    } catch (error) {
        if(typeof window.showCustomAlert === 'function') {
